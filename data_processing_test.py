...
from feature_engineering import bin_to_num, cat_to_cal ,one_hot_encoding

ingest_data = IngestData()
df = ingest_data.get_data("ols-regression-challenge-data/data/cancer_reg.csv")

constant_columns = find_constant_columns(df)
print("Column that contain a single value: ",constant_columns)
...
df = drop_and_fill(df)
print(df.shape)
df.to_csv("ols-regression-challenge-data/data/cancer_reg_processed.csv",index=False)
